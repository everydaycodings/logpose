...
export function AlbumEditForm({ data }: { data: Data }) {
  ...
  async function onSubmit(e: React.FormEvent) {
    e.preventDefault()
    setSaving(true)
    const res = await updateAlbum(data.id, { title, year })
    setSaving(false)
    if (res?.ok) {
      toast.success("Saved")
  ...
  async function uploadCover(file: File) {
    ...
    const res = await fetch(`/api/albums/${data.id}/cover`, {
      method: "POST",
      body: fd,
    })
    if (res.ok) {
  ...
  return (
    <form onSubmit={onSubmit} className="flex flex-col gap-6 sm:flex-row">
      <div className="flex w-44 shrink-0 flex-col gap-2">
        <Cover
          key={coverBust}
          coverSrc={`/api/albums/${data.id}/cover?v=${coverBust}`}
          alt={title}
          rounded="rounded-2xl"
        />
        <Button
